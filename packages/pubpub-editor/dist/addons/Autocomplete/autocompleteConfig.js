...
function insertMention(_ref2) {
	...
	var textnode = view.state.schema.text(text);
	var transaction = view.state.tr.replaceRangeWith(start, end, view.state.schema.nodes.mention.create({ url: url, type: type }, textnode));
	...
	switch (selectedObject.itemType) {
		case 'file':
			...
			if (selectedObject.slug) {
				...
				text = filename;
				insertMention({ view: editor.view, start: start, end: end, text: text, url: url, type: 'file' });
			} else {
