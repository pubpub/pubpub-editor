...
class EmbedView extends ReactView {
  ...
  renderElement(domChild) {
    ...
    const nodeType = this.view.state.schema.nodes.embed;
  ...
  createCaption() {
    ...
    const textnode = this.view.state.schema.text('Enter caption');
    const captionNode = this.view.state.schema.nodes.caption.create({}, textnode);
  ...
  updateCaption(txt) {
    ...
    const slice = new Slice(Fragment.from(this.view.state.schema.text(txt)), 0, 0);
