...
var findNodeByFunc = function findNodeByFunc(doc, nodeEval) {
	var foundNode = null;
	var nodeMatch = function nodeMatch(node, index, parent) {
		if (nodeEval(node)) {
			foundNode = { node: node, index: index };
		}
	};
...
var findNodesByFunc = function findNodesByFunc(doc, nodeEval) {
	var foundNodes = [];
	var nodeMatch = function nodeMatch(node, index, parent) {
		if (nodeEval(node)) {
			foundNodes.push({ node: node, index: index });
		}
	};
	doc.descendants(nodeMatch);
	return foundNodes;
