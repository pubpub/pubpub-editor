...
export class ModServerCommunications {
	...
	createWSConnection = () => {
		...
		try {
			ws = new window.WebSocket(`${websocketProtocol}//${wsServer}/ws/doc/${this.editor.doc_id}?user=${this.editor.username}&token=${this.editor.token}&avatar_url=${this.editor.img}&random=${randomInt}&repo_id=${this.editor.repoID}`);
			this.ws = ws;
