...
var renderSubLoop = function renderSubLoop(item, meta) {
	...
	var content = item.map(function (node, index) {
		switch (node.type) {
			...
			case 'equation':
				var equationText = node.content && node.content.length >= 1 ? node.content[0].text : '';
				return _react2.default.createElement(_components.LatexRender, { key: index, value: equationText, block: false });
			case 'block_equation':
				var blockEquationText = node.content && node.content.length >= 1 ? node.content[0].text : '';
				return _react2.default.createElement(_components.LatexRender, { key: index, value: blockEquationText, block: true });
			case 'iframe':
