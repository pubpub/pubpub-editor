import { CitationsRender, EmbedRender, IframeRender, LatexRender, ReferenceRender } from './components';
...
const renderSubLoop = function(item, meta) {
    ...
    case 'article':
		...
		case 'equation':
			const equationText = (node.content && node.content.length >= 1) ? node.content[0].text : '';
			return <LatexRender key={index} value={equationText} block={false} />
		case 'block_equation':
			const blockEquationText = (node.content && node.content.length >= 1) ? node.content[0].text : '';
